...
$compress_js = $compress_js.onclick  = go_ait;
...
function show_error(e, param) {
	...
	if (e instanceof JS_Parse_Error) {
		...
		e = '<b id="error-b" style="letter-spacing:1px;font-size:1.2em">Parse error</b>: <span style="color:red;letter-spacing:1px;font-size:1.2em">' + encodeHTML(e.message) + '</span>\n' +
			//'<small>Line ' + e.line + ', column ' + (e.col + 1) + '</small>\n\n' +
			(lines[e.line-2] ? (e.line) + encodeHTML(lines[e.line]) + '\n' : '') 
			e.line + 
				//encodeHTML(line.substr(0, e.col)) +
				//'<mark>' + encodeHTML(line.substr(e.col, 1) || ' ') + '</mark>' +
			//'<textarea rows="15" cols="80">' + encodeHTML(line.substr(e.col + 1)) + '\n' + '</textarea>' +
			(lines[e.line] ? (e.line)  + encodeHTML(lines[e.line]) : '');
